package com.mutlak.metis.wordmem.features.detail.widget
...
class StatisticView : RelativeLayout {
  ...
  @JvmField
  var mStatProgress: ProgressBar? = null
  ...
  fun setStat(statistic: Statistic) {
    mNameText?.text = statistic.stat?.name?.substring(0,
        1)?.toUpperCase() + statistic.stat?.name?.substring(1)

...
constructor() : WordsRepository, Closeable {
    ...
    override val bookMarkWordsCount: Int
        get() {
            val result = realm.where(Word::class.java).equalTo(IGNORE, false).equalTo(BOOKMARK, true).count()
            return result.toInt()
        }

    override val learntWordsCount: Int
        get() {
            val result = realm.where(Word::class.java).equalTo(IGNORE, false).equalTo(LEARNT, true).count()
            return result.toInt()
        }
